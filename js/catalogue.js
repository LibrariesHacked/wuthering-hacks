$(function () {
  ...
  if (window.location.pathname.indexOf('catalogue') != -1) {
    ...
    $.when(
      ...
      var branchLookup = {}
      $.each(branches, function (i, x) {
        branchLookup[x.id] = x.branch
      })
      var catLookup = {}
      $.each(categories, function (i, x) {
        catLookup[x.id] = x.category
      })
      ...
      var catalogueDateDim = catalogueNdx.dimension(function (d) {
        return d.year_added
      })
      var itemsTotal = removeEmpty(
        catalogueDateDim.group().reduceSum(function (d) {
          return d['count']
        })
      )
      var priceTotal = removeEmptyCost(
        catalogueDateDim.group().reduceSum(function (d) {
          return d['price']
        })
      )
      var issuesTotal = removeEmpty(
        catalogueDateDim.group().reduceSum(function (d) {
          return d['issues']
        })
      )
      var renewalsTotal = removeEmpty(
        catalogueDateDim.group().reduceSum(function (d) {
          return d['renewals']
        })
      )
      ...
      var itemsNumGroup = catalogueNdx.groupAll().reduceSum(function (d) {
        return d['count']
      })
      itemsNumberDisplay
      ...
      var issuesNumGroup = catalogueNdx.groupAll().reduceSum(function (d) {
        return d['issues']
      })
      issuesNumberDisplay
        .valueAccessor(function (d) {
      ...
      var renewalsNumGroup = catalogueNdx.groupAll().reduceSum(function (d) {
        return d['renewals']
      })
      ...
      var costNumGroup = catalogueNdx.groupAll().reduceSum(function (d) {
        return d['price']
      })
      costNumberDisplay
        .valueAccessor(function (d) {
      ...
      var catalogueYearDim = catalogueNdx.dimension(function (d) {
        return +d.year_added
      })
      var catalogueYearTotal = catalogueYearDim.group().reduceSum(function (d) {
        return d.count
      })
      catalogueYearChart
      ...
      catalogueYearChart.on('renderlet', function (chart) {
        chart
          .selectAll('g.axis.x g.tick text')
          .attr('transform', 'translate(-13, 20) rotate(270)')
      })
      ...
      var catalogueDayDim = catalogueNdx.dimension(function (d) {
        return d.day
      })
      var catalogueDayTotal = catalogueDayDim.group().reduceSum(function (d) {
        return d['count']
      })
      ...
      var catalogueCategoryDim = catalogueNdx.dimension(function (d) {
        return d.category
      })
      var catalogueCategoryTotal = removeOther(
        catalogueCategoryDim.group().reduceSum(function (d) {
          return d['count']
        })
      )
      ...
      catalogueCategoryChart.on('renderlet', function (chart) {
        chart
          .selectAll('g.axis.x g.tick text')
          .attr('transform', 'translate(-13, 7) rotate(270)')
      })
      ...
      var catalogueBranchDim = catalogueNdx.dimension(function (d) {
        return d.branch
      })
      var catalogueBranchTotal = catalogueBranchDim
        .group()
      ...
      catalogueRowBranchChart.on('renderlet', function (chart) {
        chart
          .selectAll('g.axis.x g.tick text')
          .attr('transform', 'translate(-11, 10) rotate(270)')
      })
      ...
      $(window).on('resize', function () {
        if ($(window).width() != width) {
          width = $(window).width()
          catalogueLineChart.width(
            document.getElementById('div-catalogue').offsetWidth
          )
          catalogueCategoryChart.width(
            document.getElementById('div-catalogue-category').offsetWidth
          )
          catalogueDayBarChart.width(
            document.getElementById('div-catalogue-day').offsetWidth
          )
          catalogueYearChart.width(
            document.getElementById('div-catalogue-year').offsetWidth
          )
